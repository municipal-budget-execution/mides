        ...
        with
            ...
            pago_ce as (
                select
                    ...
                    round(safe_cast(valor_nota_pagamento as float64), 2) as valor_liquido_recebido,
            ...
            pagamento_es as (
                select
                    ...
                    safe_cast (concat(NumeroEmpenho, ' ', CodigoUnidadeGestora, ' ', concat(CodigoOrgao, CodigoUnidadeOrcamentaria), ' ', id_municipio, ' ', (substring(AnoEmpenho,3,2))) as string) as id_empenho_bd,
                    safe_cast (null as string) as id_empenho,
                    safe_cast (NumeroEmpenho as string) as numero_empenho,
                    safe_cast (null as string) as id_liquidacao_bd,
                    safe_cast (null as string) as id_liquidacao,
                    safe_cast (null as string) as numero_liquidacao,
                    ...
                    safe_cast (CodigoCredorFornecedor as string) as documento_credor,
                    safe_cast (NomeCredorFornecedor as string) as nome_credor,
                    safe_cast ((case when safe_cast (RPPPago as float64) > 0 then 1 else 0 end) as bool) as indicador_restos_pagar,
                    safe_cast (CodigoGrupoFonte as string) as fonte,
                    round(safe_cast(0 as float64), 2) as valor_inicial,
                    round(safe_cast(0 as float64), 2) as valor_anulacao,
                    round(safe_cast(0 as float64), 2) as valor_ajuste,
                    round(safe_cast (replace (Paga, ',', '.')as float64),2) as valor_final,
                    round(safe_cast (replace (Paga, ',', '.')as float64),2) as valor_liquido_recebido
          ...
          pagamento_go as (
              select
                    ...
                    safe_cast (null as string) as id_pagamento,
                    safe_cast (null as string) as numero,
                    safe_cast (credor_id as string) as documento_credor,
                    ...
                    round(safe_cast(0 as float64), 2) as valor_anulacao,
                    round(safe_cast(0 as float64), 2) as valor_ajuste,
                    round(safe_cast (replace (total_op, ',', '.')as float64),2) as valor_final,
                    round(safe_cast (replace (total_op, ',', '.')as float64),2) as valor_liquido_recebido
            ...
            pagamento_pe as (
                select
                    ...
                    safe_cast(concat(trim(idempenho), ' ', trim(p.numeroempenho), ' ', trim(id_unidade_gestora), ' ',(right(anoreferencia, 2))) as string) as id_empenho_bd,
                    safe_cast(null as string) as id_liquidacao,
                    safe_cast(null as string) as numero_liquidacao,
                    safe_cast(null as string) as id_pagamento_bd,
                    safe_cast(null as string) as id_pagamento,
          ...
          empenho_ro as (
              select
                    ...
                    safe_cast (concat(empNumero, ' ',empCodigoEntidade, ' ', id_municipio, ' ', (substring(empData,-2))) as string) as id_empenho_bd,
                    safe_cast(empCnpjCpf as string) as documento_credor
          ...
          pago_ro as (
                select
                    min(safe_cast(substring(pagDataPagamento, 1,4) as int64)) as ano,
                    min(safe_cast(substring(pagDataPagamento, 6, 2) as int64)) as mes,
                    min(safe_cast(pagDataPagamento as date)) as data,
                    'RO' as sigla_uf,
                    safe_cast(id_municipio as string) as id_municipio,
                    safe_cast(null as string) as orgao,
                    safe_cast(null as string) as id_unidade_gestora,
                    safe_cast (concat(pagEmpenhoNumero, ' ',pagCodigoEntidade, ' ', id_municipio, ' ', (substring(pagDataPagamento,-2))) as string) as id_empenho_bd,
                    safe_cast(null as string) as id_empenho,
                    safe_cast(pagEmpenhoNumero as string) as numero_empenho,
                    safe_cast (concat(pagNumeroLiquidacao, ' ', pagEmpenhoNumero, ' ',pagCodigoEntidade, ' ', id_municipio, ' ', (substring(pagDataPagamento,-2))) as string) as id_liquidacao_bd,
                    safe_cast(null as string) as id_liquidacao,
                    safe_cast(pagNumeroLiquidacao as string) as numero_liquidacao,
                    safe_cast(concat(pagNumeroPagamento, ' ', pagNumeroLiquidacao, ' ', pagEmpenhoNumero, ' ',pagCodigoEntidade, ' ', id_municipio, ' ', (substring(pagDataPagamento,-2))) as string) as id_pagamento_bd,
                    safe_cast(null as string) as id_pagamento,
                    safe_cast(pagNumeroPagamento as string) as numero,
                    safe_cast(null as string) as nome_credor,
                    safe_cast(null as bool) as indicador_restos_pagar,
                    safe_cast(null as string) as fonte,
                    round(safe_cast(0 as float64), 2) as valor_inicial,
                    round(safe_cast(0 as float64), 2) as valor_anulacao,
                    round(safe_cast(0 as float64), 2) as valor_ajuste,
                    round(safe_cast (concat(pagSinalPagamento, pagValorPagamento) as float64),2)  as valor_final,
                    round(safe_cast (concat(pagSinalPagamento, pagValorPagamento) as float64),2)  as valor_liquido_recebido,
                from `basedosdados-dev.world_wb_mides_staging.raw_pagamento_ro` p
                left join `basedosdados-dev.world_wb_mides_staging.aux_municipio_ro` a on ltrim(regexp_replace(p.pagCodigoEntidade, r'\..', ''), '0')=trim(a.id_entidade)
                group by all
          ),
          pagamento_ro as (
              select
                  ...
                  p.valor_liquido_recebido
              from pago_ro p
              left join empenho_ro e on p.id_empenho_bd = e.id_empenho_bd
          ),
            pago_rs as (
                select
            ...
            empenho_to as (
                select
                    min(safe_cast (concat(nr_empenho, ' ', orgao, ' ', unidade_orcamentaria, ' ', municipio, ' ', (substring(exercicio,3,2))) as string)) as id_empenho_bd,
                    safe_cast (idcredor as string) as documento_credor,
                from `basedosdados-dev.world_wb_mides_staging.raw_empenho_to`
                group by all
            ),
            frequencia_to as (
                select id_empenho_bd, count(id_empenho_bd) as frequencia_doc
                from empenho_to
                group by 1
                order by 2 desc 
            ),
            pago_to as (
                select
                    safe_cast (exercicio as int64) as ano,
                    safe_cast ((substring(data, -7, 2)) as int64) as mes,
                    safe_cast(
                                            concat(
                                                substring(data, -4),
                                                '-',
                    ...
                    sum(safe_cast (concat(sinal, valor) as float64)) as valor_liquido_recebido
                from `basedosdados-dev.world_wb_mides_staging.raw_pagamento_to` 
                group by all                    
            ...
            pagamento_to as (
                select
                    ...
                    p.numero,
                    p.nome_credor,
                    (case when frequencia_doc > 1 then (safe_cast (null as string)) else e.documento_credor end) as documento_credor,
                    safe_cast(null as bool) as indicador_restos_pagar,
                    safe_cast(null as string) as fonte,
                    round(safe_cast(0 as float64), 2) as valor_inicial,
                    round(safe_cast(0 as float64), 2) as valor_anulacao,
                    ...
                    p.valor_liquido_recebido
                from pago_to p
                left join empenho_to e on e.id_empenho_bd=p.id_empenho_bd
                left join frequencia_to f on f.id_empenho_bd=f.id_empenho_bd
            )
      select *
        from pagamento_ce
        union all
        ...
        union all
        (select * from pagamento_es)
        union all
        (select * from pagamento_go)
        union all
