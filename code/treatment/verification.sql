        ...
        with
            ...
            liquidacao_es as (
                select
                    ...
                    round(safe_cast (0 as float64),2) as valor_anulacao,
                    round(safe_cast (0 as float64),2) as valor_ajuste,
                    round(safe_cast (replace (Liquidada, ',', '.')as float64),2) as valor_final,
            ...
            liquidacao_go as (
                select
                      ...
                      round(safe_cast (0 as float64),2) as valor_anulacao,
                      round(safe_cast (0 as float64),2) as valor_ajuste,
                      round(safe_cast (total_liquidado as float64),2) as valor_final,
            ...
            liquidacao_pe as (
                select
                    ...
                    safe_cast(concat(trim(idempenho), ' ', trim(numeroempenho), ' ', trim(id_unidade_gestora), ' ',(right(anoreferencia, 2))) as string) as id_empenho_bd,
            ...
            liquidacao_ro as (
                select
                    min(safe_cast(substring(liqDataLiquidacao, 1,4) as int64)) as ano,
                    min(safe_cast(substring(liqDataLiquidacao, 6, 2) as int64)) as mes,
                    min(safe_cast(liqDataLiquidacao as date)) as data,
                    'RO' as sigla_uf,
                    safe_cast (id_municipio as string) as  id_municipio,
                    safe_cast (null as string) as  orgao,
                    safe_cast (null as string) as id_unidade_gestora,
                    safe_cast (concat(liqEmpenhoNumero, ' ',liqCodigoEntidade, ' ', id_municipio, ' ', (substring(liqDataLiquidacao,3,2))) as string) as id_empenho_bd,
                    safe_cast (null as string) as id_empenho,
                    safe_cast (liqEmpenhoNumero as string) as numero_empenho,
                    safe_cast (concat(liqNumeroLiquidacao, ' ', liqEmpenhoNumero, ' ',liqCodigoEntidade, ' ', id_municipio, ' ', (substring(liqDataLiquidacao,3,2))) as string) as id_liquidacao_bd,
                    safe_cast (null as string) as id_liquidacao,
                    safe_cast (liqNumeroLiquidacao as string) as numero,
                    safe_cast (null as string) as nome_responsavel,
                    safe_cast (liqCnpjCpf as string) as documento_responsavel,
                    safe_cast (null as bool) as indicador_restos_pagar,
                    round(safe_cast (0 as float64),2) as valor_inicial,
                    round(safe_cast (0 as float64),2) as valor_anulacao,
                    round(safe_cast (0 as float64),2) as valor_ajuste,
                    round(safe_cast (concat(liqSinal, liqValorLiquidacao) as float64),2) as valor_final,
                from `basedosdados-dev.world_wb_mides_staging.raw_liquidacao_ro` l
                left join `basedosdados-dev.world_wb_mides_staging.aux_municipio_ro` a on ltrim(regexp_replace(l.liqCodigoEntidade, r'\..', ''), '0')=trim(a.id_entidade)
                group by all
            ),
            liquidado_rs as (
                select
            ...
            liquidacao_to as (
                select
                    ...
                    safe_cast(null as string) as id_liquidacao,
                    safe_cast(nr_liquidacao as string) as numero,
                    safe_cast(nome_credor as string) as nome_responsavel,
                    safe_cast(credor as string) as documento_responsavel,
                    safe_cast(null as bool) as indicador_restos_pagar,
                    round(safe_cast(0 as float64), 2) as valor_inicial,
                    round(safe_cast(0 as float64), 2) as valor_anulacao,
                    round(safe_cast(0 as float64), 2) as valor_ajuste,
                    sum(safe_cast (concat(sinal, valor) as float64)) as valor_final,
                from `basedosdados-dev.world_wb_mides_staging.raw_liquidacao_to`
                group by all
            )
      select *
        from liquidacao_ce
        ...
        union all
        (select * from liquidacao_es)
        union all
        (select * from liquidacao_go)
        union all
        (select * from liquidacao_mg)
        union all
